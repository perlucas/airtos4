...
from a3c.training_session import TrainingSession
...
class AirtosHyperModel(kt.HyperModel):
    ...
    def build(self, hp):
        # Compute the number of layers for the Actor
        actor_layers = []
        num_layers = hp.Choice("actor_layers", [6, 9, 12, 15])
        layer_units = hp.Int("actor_layer_units", min_value=50, max_value=400, step=50)
        for _ in range(num_layers):
            actor_layers.append(layer_units)
        actor_layers = tuple(actor_layers)

        # Define the number of units for the critic
        critic_layers = []
        num_layers = hp.Choice("critic_layers", [6, 9, 12, 15])
        layer_units = hp.Int("critic_layer_units", min_value=50, max_value=400, step=50)
        for _ in range(num_layers):
            critic_layers.append(layer_units)
    ...
    def run(self, hp, model, trial, *args, **kwargs):
        ...
        training_session = TrainingSession(agent=agent)
        final_stats = training_session.run(
            num_episodes=PARAM_NUM_ITERATIONS,
            episodes_per_job=PARAM_EPISODES_PER_JOB,
            num_workers=4,
            log_dir=LOG_DIR
        )
